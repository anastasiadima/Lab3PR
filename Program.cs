...
namespace Raft
{
    ...
    class Program
    {
        ...
        {
            public IDictionary<PeerId, KeyValueStore<int>> listOfNodes { get; set; }

            public PeerId GetLeaderNode()
            {
                var peersIds = listOfNodes.Values;
                var peer = peersIds.First(server => server.Server.IsLeader() == true)._id;
